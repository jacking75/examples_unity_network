...
namespace LobbyServer
{
    public class LobbyServer : AppServer<ClientSession, EFBinaryRequestInfo>
    {
        ...
        {
            Lobby.NetSendFunc = this.SendData;
            LobbyMgr.CreateRooms();

            MainPacketProcessor = new PacketProcessor();
            MainPacketProcessor.CreateAndStart(LobbyMgr.GetLobbyList(), this);
            

            //MainPacketProcessor.DevMQTest();
        ...
        public bool SendData(string sessionID, UInt16 packetID, byte[] bodyData)
        {
            ...
            catch(Exception ex)
            {
                ...
                LobbyServer.MainLogger.Error($"{ex.ToString()},  {ex.StackTrace}");
