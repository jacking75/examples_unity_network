...
public class Chat : MonoBehaviour
{
	...
	void UpdateChatting()
	{
		...
		if (packet.PacketID == (UInt16)PACKET_ID.PACKET_ID_CHAT_NTF)
		{
			var responsePkt = MessagePackSerializer.Deserialize<CSMsgPackPacket.ChatNtfPkt>(packet.BodyData);

			string message = $"[{responsePkt.UserID}]: {responsePkt.Msg}";
	...
	void ChattingGUI()
	{
		...
		if (isSent == true)
		{
			...
			PostSendPacket(PACKET_ID.PACKET_ID_CHAT_REQ, buffer);
